...
weeks_flooded <- 
  DSMhabitat::weeks_flooded %>% 
  reshape::melt() %>%
  as.data.frame() %>%
  setNames(c('watershed', 'month', 'year', 'weeks_flooded')) %>% 
  mutate(month = match(month, month.abb), 
         date = ymd(paste(year, month, 1, sep = '-')),
         weeks_flooded = as.factor(weeks_flooded)) 

habitat_with_weeks_flooded <- habitat %>% left_join(weeks_flooded, by = c("region" = "watershed", "date")) %>% glimpse()


write_rds(habitat_with_weeks_flooded, 'data/habitat.rds')
...
# No longer scaling habitat with new inputs
